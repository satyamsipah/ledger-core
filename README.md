...
live in [docs/DECISIONS.md](docs/DECISIONS.md) — fifty-six numbered
...
| **Any production deployment.** Single-node Docker Compose only; no Kubernetes manifests, no multi-region, no managed Postgres/Kafka. | Deliberately out of scope for every phase so far — this repository proves the ledger's correctness and its performance envelope on one machine; deploying it is separate, later work. |
...
- [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) — how every subsystem
  ...
  compensation paths
- [docs/DECISIONS.md](docs/DECISIONS.md) — fifty-six decisions across
