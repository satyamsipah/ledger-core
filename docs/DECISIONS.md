...
## Phase 8 continued — Deployment configs, Part 2

Prepared, not deployed: no cloud accounts exist for this project yet.
Everything below is verified statically -- `docker build`, `helm lint`,
`helm template` piped through `kubeconform -strict`, `actionlint` -- not
against a live cluster or VPS.

### D57. Distroless production images, and a dedicated healthcheck binary

**Decided.** `deploy/Dockerfile.prod` builds every service exactly as
`deploy/Dockerfile` does (one shared, multi-stage Dockerfile, `SERVICE`
build arg), onto `gcr.io/distroless/static-debian12:nonroot` instead of
`alpine`. `deploy/Dockerfile` itself is untouched -- local dev keeps its
wget-based Compose healthcheck, which needs the shell and busybox
distroless deliberately does not ship.

**Rejected: alpine for production too.** Simpler -- no new binary, `wget`
keeps working -- but keeps a shell and a package manager in every
production container. For a payments ledger, that is not a size
optimisation so much as removing the toolset an attacker with code
execution would otherwise have for free. The trade-off accepted in
exchange: no `docker exec sh` into a running production container.
`kubectl debug` (an ephemeral container with its own full toolset,
attached on demand) and this codebase's own metrics/tracing are the
intended path instead.

**A real bug, found by actually building the image, not by reading the
Dockerfile.** The first attempt baked a per-service health-check port into
`HEALTHCHECK`'s `CMD` array via the same `${ARG}` substitution
`ENV LEDGER_SERVICE=${SERVICE}` already uses successfully one line above
it. `docker inspect` showed the literal, unexpanded string
`"${HEALTHCHECK_PORT}"` in the built image's config -- Docker's build-arg
substitution is documented to apply to a specific instruction list (`ADD`,
`COPY`, `ENV`, `EXPOSE`, `FROM`, `LABEL`, `STOPSIGNAL`, `USER`, `VOLUME`,
`WORKDIR`), and `HEALTHCHECK` is not on it, silently, with no build
warning. Fixed by moving the value through `ENV HEALTHCHECK_URL=...`
(which *is* on the list) and having `cmd/healthcheck` read
`$HEALTHCHECK_URL` from its own process environment instead of a baked-in
CLI flag -- the container runtime hands every process its environment
regardless of exec form, so this needed no shell to expand anything,
unlike the `CMD`-array `${VAR}` that does not exist.

**Why a dedicated binary rather than reusing an existing one.**
`cmd/healthcheck` is a few hundred lines: one HTTP GET, one exit code.
Every image copies the identical binary -- one more `COPY` in
`Dockerfile.prod`, not a new Dockerfile -- so the eight service images
still cannot drift apart from each other in how they report health,
matching the reasoning `deploy/Dockerfile`'s own top comment already gives
for sharing one Dockerfile in the first place.

**What this does NOT solve.** Kubernetes' own liveness/readiness probes
need none of this: `kubelet` performs the HTTP check from outside the
container over the pod network, no shell involved either way. The
healthcheck binary exists specifically for Docker Compose's in-container
`HEALTHCHECK`, which `docker-compose.prod.yml` still needs for its
`depends_on: condition: service_healthy` ordering.

### D58. The Helm chart is the source of truth; deploy/k8s/ is generated, not hand-written

**Decided.** `deploy/helm/ledger-core/` is one parameterized chart
covering all five stateless services (`api`, `outbox-publisher`,
`projector`, `saga-orchestrator`, `reconciler`) plus two hook Jobs
(`migrate`, `kafka-init`). `deploy/k8s/ledger-core.yaml` is that chart's
own `helm template` output, checked in as a static, kubectl-apply-able
reference for anyone without Helm, regenerated by one documented command,
never hand-edited.

**Rejected: two independently maintained manifest sets** -- full raw
manifests in `deploy/k8s/` AND a separate Helm chart, each hand-written.
More files, and a real, ongoing risk the two quietly disagree (one
updated, the other not) -- the identical failure mode this same session's
documentation pass (Phase 8 Part 3) found and fixed twice over in
`docs/RUNBOOK.md` and `docs/DECISIONS.md` itself, just at the
config-file layer instead of the prose layer.

**One template per resource type, `range`d over `values.yaml`'s
`services` map, not five copies of each.** `templates/deployment.yaml`,
`service.yaml`, `hpa.yaml` and `pdb.yaml` are each one file that emits one
resource per enabled service. Adding a sixth service is one `values.yaml`
entry; four hand-copied templates per service would have been five ways
for the sixth service to accidentally end up different from the first
five.

**Scope: no Postgres, Kafka, Prometheus or Grafana.** A real cluster's
Postgres and Kafka are a managed service or a dedicated operator
(CloudNativePG, Strimzi, ...) with their own backup and failover story --
a hand-rolled `StatefulSet` here would be strictly worse than either.
`postgres.dsn` and `kafka.brokers` are chart *inputs* for exactly this
reason. Prometheus/Grafana are conventionally `kube-prometheus-stack` or
similar, which discovers this chart's Services via
`app.kubernetes.io/*` labels without this chart needing to know it exists.

**Readiness probes check what each service actually holds a connection
to, not a uniform lie.** `outbox-publisher` and `projector` hold a live
`*kgo.Client` (see D59's `internal/kafka.Checker`) and their `/readyz`
reflects that; `api`, `saga-orchestrator` and `reconciler` do not touch
Kafka directly and their `/readyz` checks Postgres only. Faking a Kafka
check on a service that cannot structurally have one would have satisfied
the letter of "a readiness probe that checks DB and Kafka connectivity"
while being false the moment anyone read the code behind it.

**Verification.** No live cluster exists to test against, so this is
proven three ways instead: `helm lint` (0 failures), `helm template`
piped through `kubeconform -strict` (22/22 resources valid against
Kubernetes' real OpenAPI schemas, not merely well-formed YAML), and
hand-inspection of the rendered output for the two properties a schema
validator cannot check -- that HPA-managed Deployments correctly omit
`spec.replicas` (present for `outbox-publisher`, `projector`,
`reconciler`; absent for `api`, `saga-orchestrator`, the two with
`hpa.enabled: true`), and that `api`'s probes target port 8080 while
every worker's target its own metrics port.

### D59. Migrations run from an embedded binary, identically on both production paths; the VPS compose defaults to polling, not Debezium

**Decided, migrations.** `cmd/migrate` links `migrations.FS`
(`migrations/embed.go`) directly via `golang-migrate`'s Go API -- the same
mechanism `test/testdb.go` already uses to migrate a fresh Testcontainers
Postgres for every test run -- rather than the standalone
`migrate/migrate:v4.17.1` CLI image `deploy/docker-compose.yml` uses with
a bind-mounted `../migrations`. `docker-compose.prod.yml`'s `migrate`
service and `deploy/helm/ledger-core`'s pre-install/pre-upgrade hook Job
both run this exact image, so a single-VPS deploy and a Kubernetes deploy
migrate through the identical code path -- not two mechanisms that could
answer "did this migrate correctly" differently -- and neither production
path needs the repository's `migrations/` directory present on the host
or in a volume: the SQL is inside the image that already has to be pulled
anyway.

**Verified against the real, already-migrated Postgres this session's
Docker stack has been running throughout Phase 8**: `go run ./cmd/migrate`
against it reports `"schema already up to date", "version":18` and exits
0; the built distroless image does the same run over the real Docker
network by name (`postgres:5432`), not merely against `localhost`.

**Decided, the outbox publisher arm.** `docker-compose.prod.yml` sets
`LEDGER_OUTBOX_PUBLISHER=polling`, not `debezium` (`deploy/docker-compose.yml`'s
default). D31 already gave the full comparison; what changes here is which
side of that trade-off a single VPS should take. Debezium needs a whole
Kafka Connect JVM process and its own connector registration step
(`connect`, `connect-init` in local dev) for one benefit -- strict
commit-order delivery over the polling arm's insertion-order delivery --
that a single-machine deployment's traffic volume does not need enough to
justify the extra service. The Helm chart takes no position on this: it
is `outbox-publisher`'s `extraEnv`, an operator's choice per environment
values file, not a chart default, because a Kubernetes cluster's Debezium
operational cost is a different (usually much lower) one.

**Kafka readiness, made honest rather than assumed.** Neither production
path could honestly claim "checks Kafka connectivity" before this phase:
`ledgerhttp.Checker` had exactly one implementation
(`internal/db.Pool`), and no service's `/readyz` reflected Kafka being
unreachable at all. `internal/kafka.Checker` wraps an existing
`*kgo.Client`'s own `Ping` -- deliberately the client each service
*already holds for its real work*, not a second connection opened only to
probe, which would answer "can a new connection reach the broker" rather
than "is the connection this process actually uses still good." Wired
into `cmd/projector` and `cmd/outbox-publisher`'s polling arm, the two
services that hold one; not into `cmd/api`, `cmd/saga-orchestrator` or
`cmd/reconciler`, which structurally do not, per D40 and this phase's own
narrower scope. `TestKafkaChecker_ReportsRealBrokerReachability` and
`TestKafkaChecker_UnreachableBrokerFailsCheck` (`test/kafka_checker_test.go`)
prove both directions against a real Redpanda container and a real
closed port, not a mock.
