...
| `GET` | `/v1/transactions` | required | — |
| `GET` | `/v1/transactions/{id}` | required | — |
| `GET` | `/v1/accounts/{id}/balance` | — | — |
| `GET` | `/v1/accounts/{id}/statement` | — | — |
| `GET` | `/v1/accounts` | required | — |
| `GET` | `/v1/accounts/{id}` | required | — |
| `POST` | `/v1/payouts` | required | required |
| `GET` | `/v1/sagas` | — | — |
| `GET` | `/v1/sagas/{id}` | — | — |
| `GET` | `/healthz`, `/readyz` | — | — |

`GET /v1/transactions` and `GET /v1/accounts` are keyset-paginated search —
the dashboard's ledger explorer and account view — and, together with their
`{id}` counterparts, are the one place read routes require auth: a search
result carries amounts and external references, the class of information
D24 already scopes idempotency responses to protect, and unlike balance or
statement it needs no UUID to begin with. See docs/DECISIONS.md D55.
...
ledger explorer (search transactions, drill into journal entries, see the
debit/credit balance of each one), an account view (balance, statement with
running balance, a temporal "as of" picker), a saga monitor (state machine,
the manual-review queue, stuck-saga highlighting), a reconciliation report
(exceptions by category, drill-down), and system health (throughput,
latency, outbox lag, invariant status).
...
view was verified in both modes; see D55 and D56 in
[docs/DECISIONS.md](docs/DECISIONS.md) for what that found, and
